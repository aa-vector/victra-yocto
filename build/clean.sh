...
docker run -it \
    ...
    vic-yocto-builder-3 bash -c "cd $(pwd)/poky && source build/conf/set_bb_env.sh && MACHINE=apq8009-robot VARIANT=perf DISTRO=victra PRODUCT=robot bitbake -c cleanall $@"
